...
|---|---:|---|---:|---:|---:|---:|---:|
| **proposed_c2f_adaptive_pd** | **0.9762** | [0.9730, 0.9790] | **0.967** | 0.0497 | **0.751** | **0.80** | 481 |
| exact_marginal_greedy | 0.9411 | [0.9363, 0.9455] | 0.928 | 0.0497 | 5.131 | 5.47 | 3284 |
| sense_sinr | 0.9783 | [0.9753, 0.9810] | 0.970 | 0.0497 | 4.595 | 4.90 | 2941 |

逐 trial 配对：

- vs `exact_marginal_greedy`：**Δ = +0.0351**，CI [0.0310, 0.0392] —— **不含 0，显著**
- vs `sense_sinr`：**Δ = −0.0021**，CI [−0.0051, +0.0009] —— **含 0，统计上不可分**
...
⚠️ **这张表是 09-14 旧判决阈值的结果**；同场景新判决实测 P_D = 0.975（MC=100）。
**P0（重跑 MC=1000 + 更新正文/fig2/3）仍未收口**，见 `MANUSCRIPT_CODE_CONSISTENCY_ALERT.md`。
...
**判定**：论文工作点达标且通信极省（0.9762 / 0.751 报告 / 0.80 ms）；
小目标不达标是**链路预算**问题，不是算法问题。

**要做的事，按优先级**：

1. 🔴 **P0（阻塞投稿，剩 16 天）**：重跑 MC=1000 主实验 → 更新 `SimulationResults.tex` + fig2/3。
   现在正文数字与代码判决不一致（描述与数据不符）。
2. **优化路线（若要救小目标）**：
