...
# -*- coding: utf-8 -*-
"""
Lagrangian-guided simplified DOTFS-ISAC cooperative sensing simulation (v10 paper-plot update).
...
V10 updates:
    - paper-ready main-comparison plotting for P_D, delay, P_D/ms, selected links,
      and actual worst-target P_D,
    - LaTeX table export for the main numerical comparison,
    - compact paper combined figures for lambda sweep, communication sweep,
      ablation, DD ablation, and robustness experiments,
    - actual per-target detection statistics and actual worst-target P_D reporting.
"""
...
class MethodResult:
    ...
    detected: int
    total_targets: int
    # Per-target actual detection indicators for this Monte Carlo trial.
    # This enables actual worst-target P_D reporting, instead of relying only on
    # D-threshold-based predicted satisfaction metrics.
...
def evaluate_detection(
    ...
    """
    detected = 0
    detected_per_target = np.zeros(cfg.Q, dtype=int)
...
def run_method_on_trial(
    ...
    detected, total_targets, fa, total_false, fa_overall, total_false_overall, detected_per_target = evaluate_detection(
...
def lambda_sweep(cfg: SimConfig, lambda_values: List[float]) -> List[Dict[str, Any]]:
    ...
    for lam in lambda_values:
        ...
        row = {
            ...
            "actual_worst_target_P_D": s_prop["actual_worst_target_P_D"],
            "actual_mean_target_P_D": s_prop["actual_mean_target_P_D"],
...
# ============================================================
# Paper-ready plotting and LaTeX table helpers
# ============================================================
...
def plot_main_comparison(summary: Dict[str, Dict[str, Any]], output_prefix: Path) -> None:
    """Generate paper-ready main-comparison figures for a single run.
    ...
    Saved figures:
        ...
        *_pd_per_ms.png
        *_selected_links.png
        *_actual_worst_pd.png
        *_paper.png  (3-panel combined figure)
    """
    if not summary:
        return
    import matplotlib.pyplot as plt
    ...
    labels = [_paper_label(m) for m in methods]
    x = np.arange(len(methods))

    def values(metric: str) -> np.ndarray:
        return np.array([float(summary[m].get(metric, np.nan)) for m in methods], dtype=float)

    def bar(metric: str, ylabel: str, suffix: str, yerr_metric: Optional[str] = None) -> None:
        plt.figure(figsize=(max(7.0, 1.15 * len(methods)), 4.8))
        vals = values(metric)
        if yerr_metric is None:
            plt.bar(x, vals)
        else:
            yerr = values(yerr_metric)
            plt.bar(x, vals, yerr=yerr, capsize=3)
        plt.xticks(x, labels, rotation=25, ha="right")
        plt.ylabel(ylabel)
        plt.grid(True, axis="y", alpha=0.3)
        _save_current_figure(output_prefix.with_name(output_prefix.name + suffix))

    bar("P_D", "Detection probability $P_D$", "_pd.png", "P_D_ci95_half_width")
    bar("T_mean_ms", "Mean exchange delay (ms)", "_delay.png")
    bar("P_D_per_ms", "$P_D$ per ms", "_pd_per_ms.png")
    bar("selected_links_mean", "Mean selected links", "_selected_links.png")
    bar("actual_worst_target_P_D", "Actual worst-target $P_D$", "_actual_worst_pd.png")

    fig, axes = plt.subplots(1, 3, figsize=(12.0, 3.7))
    panel_metrics = [
        ("P_D", "Detection probability $P_D$", "P_D_ci95_half_width"),
        ("T_mean_ms", "Mean exchange delay (ms)", None),
        ("P_D_per_ms", "$P_D$ per ms", None),
    ]
    for ax, (metric, ylabel, err_metric) in zip(axes, panel_metrics):
        vals = values(metric)
        if err_metric is None:
            ax.bar(x, vals)
        else:
            ax.bar(x, vals, yerr=values(err_metric), capsize=3)
        ax.set_xticks(x)
        ax.set_xticklabels(labels, rotation=25, ha="right")
        ax.set_ylabel(ylabel)
        ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(output_prefix.with_name(output_prefix.name + "_paper.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def write_main_summary_latex(summary: Dict[str, Dict[str, Any]], path: Path) -> None:
    ...
    for m in methods:
        ...
        lines.append(
            "        "
            + f"{_latex_escape(_paper_label(m))} & {float(s['P_D']):.4f} & {float(s['P_FA']):.4f} & "
...
def plot_lambda_sweep_paper(rows: List[Dict[str, Any]], output_prefix: Path) -> None:
    """Generate a compact 2-panel paper figure for lambda sweep."""
    if not rows:
        return
    import matplotlib.pyplot as plt
    lambda_vals = np.array([r["lambda_cost"] for r in rows], dtype=float)
    ...
    axes[0].plot(lambda_vals, links, marker="o")
    ...
    for x, y, lam in zip(T_ms, P_D, lambda_vals):
        axes[1].annotate(f"{lam:g}", (x, y), textcoords="offset points", xytext=(4, 4), fontsize=8)
...
def plot_comm_sweep_paper(rows: List[Dict[str, Any]], output_prefix: Path) -> None:
    """Generate a compact 3-panel paper figure for communication-constraint sweep."""
    if not rows:
        return
    import matplotlib.pyplot as plt
    ...
    for method in methods:
        axes[0].plot(x, get_vals(method, "P_D"), marker="o", label=_paper_label(method))
    ...
    for R in R_vals:
        match = [r for r in rows if np.isclose(float(r["R_min_mbps"]), R, rtol=0.0, atol=1e-9)]
        feasible_vals.append(float(match[0]["comm_feasible_edge_ratio_mean"]) if match else np.nan)
    axes[1].plot(x, feasible_vals, marker="o")
    axes[1].set_xlabel("$R_{\\min}$ (Mbit/s)")
    axes[1].set_ylabel("Feasible edge ratio")
    axes[1].grid(True, alpha=0.3)

    for method in methods:
        axes[2].plot(x, get_vals(method, "selected_rate_mean_mbps"), marker="o", label=_paper_label(method))
...
def plot_ablation_paper(rows: List[Dict[str, Any]], output_prefix: Path) -> None:
    """Generate compact fair-ablation paper figure.

    This uses actual P_D, delay, and P_D/ms as main metrics.  The D-based
    worst-target satisfaction metric is intentionally not included in the paper
    figure because it can be over-optimistic when communication-error
    ...
    labels = [_paper_label(str(r["variant"])) for r in rows]
    x = np.arange(len(rows))

    fig, axes = plt.subplots(1, 3, figsize=(12.0, 3.6))
    metrics = [
        ("P_D", "Detection probability $P_D" + "$"),
        ("T_mean_ms", "Mean exchange delay (ms)"),
        ("P_D_per_ms", "$P_D$ per ms"),
    ]
    for ax, (metric, ylabel) in zip(axes, metrics):
        vals = np.array([float(r[metric]) for r in rows], dtype=float)
        if metric == "P_D" and "P_D_ci95_half_width" in rows[0]:
    ...
    # Optional actual weak-target figure, safer than D-based worst-satisfaction.
    if "actual_worst_target_P_D" in rows[0]:
        ...
        plt.grid(True, axis="y", alpha=0.3)
        _save_current_figure(output_prefix.with_name(output_prefix.name + "_actual_worst_pd.png"))


def plot_dd_ablation_proposed_only(rows: List[Dict[str, Any]], output_prefix: Path) -> None:
    """Generate compact proposed-only DD ablation figure for a 6-page paper."""
    if not rows:
        return
    import matplotlib.pyplot as plt
    prop_rows = [r for r in rows if str(r.get("method", "")) == "proposed_lagrangian"]
    if not prop_rows:
        return
    labels = [_paper_label(str(r["variant"])) for r in prop_rows]
    x = np.arange(len(prop_rows))
    fig, axes = plt.subplots(1, 3, figsize=(12.0, 3.6))
    metrics = [
        ("P_D", "Detection probability $P_D$"),
        ("feasible_links_mean", "Feasible links/trial"),
        ("P_D_per_ms", "$P_D$ per ms"),
    ]
    for ax, (metric, ylabel) in zip(axes, metrics):
        ax.bar(x, [float(r[metric]) for r in prop_rows])
        ax.set_xticks(x)
        ax.set_xticklabels(labels, rotation=25, ha="right")
        ax.set_ylabel(ylabel)
        ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(output_prefix.with_name(output_prefix.name + "_proposed_paper.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)


def plot_comm_error_robustness_paper(rows: List[Dict[str, Any]], output_prefix: Path) -> None:
    """Generate compact robustness paper figure with P_D and P_D/ms only."""
    if not rows:
        return
    import matplotlib.pyplot as plt
    robustness_type = str(rows[0].get("robustness_type", "condition"))
    methods: List[str] = []
    conditions: List[Any] = []
    for r in rows:
        m = str(r["method"])
    ...
    for method in methods:
        axes[0].plot(x, get_vals(method, "P_D"), marker="o", label=_paper_label(method))
        axes[1].plot(x, get_vals(method, "P_D_per_ms"), marker="o", label=_paper_label(method))
    for ax in axes:
        if robustness_type == "residual_direct":
            ax.set_xscale("log")
        elif robustness_type == "comm_model":
        ...
        ax.set_xlabel(robustness_type)
        ax.grid(True, alpha=0.3)
    axes[0].set_ylabel("Detection probability $P_D$")
    axes[1].set_ylabel("$P_D$ per ms")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--main-plot-prefix", type=str, default=None, help="optional output prefix for main comparison figures from a single run")
    p.add_argument("--summary-tex", type=str, default=None, help="optional LaTeX table output path for the single-run main summary")
    ...
    p.add_argument(
        ...
        type=float,
        nargs="+",
        default=[0.0005,0.001,0.003,0.005,0.01,0.02,0.05,0.1,0.2],
...
def main() -> None:
    ...
    if args.comm_sweep:
        ...
        if args.comm_sweep_plot_prefix is not None:
            ...
            plot_comm_sweep_paper(rows, plot_prefix)
    ...
    if args.fair_ablation:
        ...
        if args.fair_ablation_plot_prefix is not None:
            ...
            plot_ablation_paper(rows, plot_prefix)
    ...
    if args.dd_ablation:
        ...
        if args.dd_ablation_plot_prefix is not None:
            ...
            plot_dd_ablation_proposed_only(rows, plot_prefix)
    ...
    if args.robustness:
        ...
        if args.robustness_plot_prefix is not None:
            ...
            plot_comm_error_robustness_paper(rows, plot_prefix)
    ...
    if args.lambda_sweep:
        ...
        if args.sweep_plot_prefix is not None:
            ...
            plot_lambda_sweep_paper(rows, plot_prefix)
    ...
    if args.main_plot_prefix is not None:
        plot_prefix = Path(args.main_plot_prefix)
        plot_main_comparison(summary, plot_prefix)
        print(f"Main comparison figures saved with prefix: {plot_prefix}")

    if args.summary_tex is not None:
        tex_path = Path(args.summary_tex)
        write_main_summary_latex(summary, tex_path)
        print(f"Main summary LaTeX table saved to: {tex_path}")
