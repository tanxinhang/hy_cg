...
def _tracking_kf_update_one(cfg: SimConfig, x_pred: np.ndarray, P_pred: np.ndarray,
                            ...
                            mode: str) -> Tuple[np.ndarray, np.ndarray, float, float, bool]:
    """Standard KF or DD reliability-weighted information-form update.

    V14 adds an optional robust innovation gate.  The gate is deliberately
    implemented before any DD-RWIF information weighting so that the trace can
    diagnose true measurement outliers instead of only DD-weight effects.

    Returns:
        x_new, P_new, gamma, innovation_d2, gated
    """
    ...
    # Innovation diagnostic/gating using the nominal measurement covariance.
    S_gate = _ensure_spd(H @ P_pred @ H.T + R)
    innovation_d2 = float(innov.T @ np.linalg.pinv(S_gate) @ innov)
    gated = bool(cfg.kf_innovation_gate and np.isfinite(innovation_d2) and innovation_d2 > float(cfg.kf_gate_chi2))
    if gated:
        action = str(cfg.kf_gate_action).lower()
        if action in ("skip", "drop", "reject"):
            return x_pred.copy(), _ensure_spd(P_pred.copy()), 1.0, innovation_d2, True
        # Default robust action: do not discard the measurement completely; make
        # it weak by inflating R.  This targets catastrophic outliers while still
        # allowing very uncertain correction.
    ...
    x_new[5] = 0.0
    return x_new, P_new, gamma, innovation_d2, gated
                    ...
                    if tracker_mode == "alpha":
                        ...
                        if cfg.tracking_reinit_lost and was_lost:
                            ...
                            if cfg.gate_reinit_measurement and cfg.kf_innovation_gate:
                                x_gate, P_gate, gamma_gate, innovation_d2, gated = _tracking_kf_update_one(
                            ...
                            x_new, P_new, gamma, innovation_d2, gated = _tracking_kf_update_one(
                                cfg, x_pred[q], P_pred[q], meas_pos, meas_vel,
                                pos_std_vec[q], vel_std_vec[q], pd_vec[q], cross_vec[q], mode_update
                            )
                            innovation_d2_vals[q] = innovation_d2
