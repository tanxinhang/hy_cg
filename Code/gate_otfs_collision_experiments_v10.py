...
V10 updates:
  - Keeps V9 multiprocessing support.
  - Fixes tracking-gate common-random-number (CRN) fairness: methods now share
    initial track errors, prediction perturbations, detection uniforms, and
    measurement noise primitives.
  - Separates evaluation RNG from tracking RNG so detector Monte Carlo does not
    desynchronize method-level tracking randomness.
  - Treats uncovered targets as coverage/loss events with NaN cross-norm rather
    than inf, and adds robust median/p90 tracking metrics.
  - Adds optional target-level OR fusion and lost-track reinitialization switches.
    ...
    for frame in range(cfg.num_frames):
        ...
        for method in methods:
            ...
            if cfg.tracking_crn:
                ...
                pred_pos, pred_vel = predict_tracks(cfg, st["track_pos"], st["track_vel"], rng_assign)
...
def run_tracking_experiment(cfg: SimConfig, out: Path) -> None:
    ...
    if {"collision_aware", "strongest"}.issubset(set(trial_metrics["method"])):
        ...
        pd.DataFrame(paired_rows).to_csv(out / "tracking_paired_diagnostics.csv", index=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD task-edge collision modeling (V10 CRN tracking)")
