...
V17 updates:
  - Adds PDA/JPDA-like probabilistic DD-peak association tracking backends
...
def _tracking_gpda_update_one(cfg: SimConfig, x_pred: np.ndarray, P_pred: np.ndarray,
    ...
    sub_mode = "dd_rwif" if str(mode).lower() in ("dd_gpda", "dd-gpda", "ddgpda") else "kf"
...
def _tracking_worker(job: Tuple[SimConfig, int]) -> Dict[str, List[Dict[str, float]]]:
    ...
    for method in methods:
        ...
        if str(cfg.tracker_mode).lower() in ("kf", "kalman", "dd_rwif", "rwif", "information", "gpda", "dd_gpda"):
            ...
            for q in range(cfg.tracking_Q):
                ...
                if detected[q]:
                    ...
                    if tracker_mode == "alpha":
                        ...
                        is_gpda_mode = tracker_mode in ("pda", "gpda", "jpda", "dd_pda", "dd_gpda", "dd_jpda", "dd-pda", "dd-gpda", "dd-jpda")
                        if is_gpda_mode:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD task-edge collision modeling (V17 GPDA tracking backend)")
    ...
    parser.add_argument("--tracker-mode", choices=["alpha", "kf", "dd_rwif", "pda", "gpda", "jpda", "dd_pda", "dd_gpda", "dd_jpda"], default="alpha", help="Tracking update model: alpha filter, standard CV-KF, DD-RWIF, PDA/GPDA-like KF, or DD-weighted GPDA-like update")
    ...
    parser.add_argument("--no-gpda-clean-candidate", action="store_true", help="Do not expose the clean candidate to GPDA when a structured polluted peak is generated; useful for ablation")
