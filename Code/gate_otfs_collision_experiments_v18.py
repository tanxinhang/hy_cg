...
V13 updates:
  - Adds temporal assignment hysteresis for collision-aware tracking gates.
  - Adds robust KF/DD-RWIF innovation gating with R-inflation or skip update.
  - Adds pollution event counts, expected pollution counts, innovation diagnostics,
    max-error diagnostics, and optional per-target measurement trace output.

V15.1 updates:
  - Fix manifest script_version from v14 to v15.1; algorithm unchanged from V15.


V18 updates:
  - Adds shared-measurement JPDA-like tracking backends (jpda_shared and
    dd_jpda_shared, with jpda/dd_jpda aliases) over a scene-level measurement
    pool.  The backend enumerates feasible joint association events per connected
    gate component, enforces one-measurement/one-track exclusivity, marginalizes
    association probabilities, and moment-matches the posterior updates.
  - Keeps V17 single-track PDA/GPDA-like backends (pda/gpda and dd_pda/dd_gpda)
    as lighter probabilistic peak-association ablations.
  - Adds JPDA joint-event and exact-enumeration diagnostics to summary/trace.

V17 updates:
  - Adds PDA/GPDA-like probabilistic DD-peak association tracking backends
    (gpda and dd_gpda) for comparison with deterministic KF/DD-RWIF updates.
  - The GPDA-style backend uses a small candidate set (clean peak and biased/
    clutter DD peak when pollution is realized) plus a missed-measurement
    hypothesis, and moment-matches the association mixture.
  - Adds GPDA association diagnostics to summary and per-target traces.

V16 updates:
  - Adds identity-aware GOSPA-like closed-loop tracking metrics that combine
    localization error and lost-track penalties.
  - Adds collision-unaware global utility assignment baselines using a capacity-
    constrained linear assignment formulation.
  - Keeps V15 robust absolute/capped innovation gates unchanged.

V15 updates:
  - Adds absolute residual, self-reported-uncertainty, and DD-cross-risk gates.
  - Computes Mahalanobis diagnostics with optional capped measurement std so that
    high-variance biased peaks no longer automatically pass the gate.
  - Adds per-target gate reason and residual diagnostics to identify catastrophic tails.
...
def assign_by_method(cfg: SimConfig, method: str, sc_pred: Scenario, U: np.ndarray, v: np.ndarray, rng: np.random.Generator, z_prev: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    if method in ("collision_aware", "collision_aware_hys", "collision_aware_switch"):
...
def _tracking_worker(job: Tuple[SimConfig, int]) -> Dict[str, List[Dict[str, float]]]:
    ...
    """
    cfg, mc = job
    base_methods = ["nearest", "strongest"]
    methods = base_methods.copy()
    if cfg.include_global_assignment_baselines:
        methods.append("global_utility")
    methods.append("collision_aware")
    if cfg.assignment_hysteresis:
        if cfg.include_global_assignment_baselines:
            methods.append("global_utility_hys")
    ...
    for frame in range(cfg.num_frames):
        ...
        for method in methods:
            ...
            method_assign = "collision_aware" if method in ("collision_aware_hys", "collision_aware_switch") else method
            z_candidate = assign_by_method(cfg, method_assign, sc_pred, U, v, rng_assign, z_prev=z_prev_for_objective)
...
def run_tracking_experiment(cfg: SimConfig, out: Path) -> None:
    ...
    if {"collision_aware", "strongest"}.issubset(set(trial_metrics["method"])):
        ...
        paired_rows = []
        for metric in ["mean_pd_target", "p05_pd_target", "mean_track_pos_rmse_m", "median_track_pos_error_m", "p90_track_pos_error_m", "gospa_pos_m", "track_loss_rate"]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD task-edge collision modeling (V18 shared-measurement JPDA tracking backend)")
    ...
    parser.add_argument("--tracker-mode", choices=["alpha", "kf", "dd_rwif", "pda", "gpda", "dd_pda", "dd_gpda", "jpda", "jpda_shared", "dd_jpda", "dd_jpda_shared"], default="alpha", help="Tracking update model: alpha filter, standard CV-KF, DD-RWIF, single-track PDA/GPDA-like, or shared-measurement JPDA-like update")
    ...
    parser.add_argument("--assignment-hysteresis", action="store_true", help="Apply temporal hysteresis to collision-aware assignments")
    parser.add_argument("--hys-rel-margin", type=float, default=0.0, help="Relative objective margin for assignment hysteresis")
    parser.add_argument("--hys-abs-margin", type=float, default=0.0, help="Absolute objective margin for assignment hysteresis")
    parser.add_argument("--no-hys-force-coverage", action="store_true", help="Do not override hysteresis when new assignment improves coverage")
    parser.add_argument("--kf-innovation-gate", action="store_true", help="Enable Mahalanobis innovation gating for KF/DD-RWIF updates")
    parser.add_argument("--kf-gate-chi2", type=float, default=13.3, help="Chi-square threshold for robust innovation gating; 13.3 is roughly 99% for effective 4-D xy/vx-vy updates")
    parser.add_argument("--kf-outlier-r-inflate", type=float, default=50.0, help="Measurement covariance inflation factor for gated outliers")
    parser.add_argument("--kf-gate-action", choices=["inflate", "skip"], default="inflate", help="Action when the innovation gate is exceeded")
    parser.add_argument("--kf-abs-pos-gate-m", type=float, default=0.0, help="Direct xy-position residual gate in meters; <=0 disables. Useful for high-variance biased peaks")
    parser.add_argument("--kf-abs-vel-gate-mps", type=float, default=0.0, help="Direct xy-velocity residual gate in m/s; <=0 disables")
    parser.add_argument("--kf-gate-pos-std-cap-m", type=float, default=0.0, help="Cap pos_std only inside Mahalanobis gate; <=0 disables")
    parser.add_argument("--kf-gate-vel-std-cap-mps", type=float, default=0.0, help="Cap vel_std only inside Mahalanobis gate; <=0 disables")
    parser.add_argument("--kf-reject-pos-std-m", type=float, default=0.0, help="Gate detected measurements whose reported pos_std exceeds this value; <=0 disables")
    parser.add_argument("--kf-reject-cross-norm", type=float, default=0.0, help="Gate detected measurements whose full_cross_norm exceeds this value; <=0 disables")
    parser.add_argument("--no-gate-reinit-measurement", action="store_true", help="Allow lost-track reinitialization to bypass innovation gating; mainly for ablation/debugging")
    parser.add_argument("--save-measurement-trace", action="store_true", help="Save per-target tracking_measurement_trace.csv for outlier diagnostics")
    parser.add_argument("--gospa-cutoff-m", type=float, default=300.0, help="Cutoff distance for the identity-aware GOSPA-like tracking metric")
    parser.add_argument("--gospa-p", type=float, default=2.0, help="p order for the GOSPA-like metric")
    parser.add_argument("--gospa-alpha", type=float, default=2.0, help="alpha denominator for missed-track penalty in the GOSPA-like metric")
    parser.add_argument("--no-gospa-normalize-by-q", action="store_true", help="Report unnormalized total GOSPA-like score instead of per-target normalized score")
    parser.add_argument("--include-global-assignment-baselines", action="store_true", help="Add collision-unaware global utility baselines to the tracking gate")
    parser.add_argument("--gpda-missed-prior", type=float, default=0.05, help="Prior mass for the missed/no-valid-measurement hypothesis in GPDA-like updates")
    parser.add_argument("--gpda-likelihood-floor", type=float, default=1e-12, help="Numerical floor for GPDA candidate likelihoods")
    parser.add_argument("--no-gpda-clean-candidate", action="store_true", help="Do not expose the clean candidate to PDA/JPDA when a structured polluted peak is generated; useful for ablation")
    parser.add_argument("--jpda-max-joint-events", type=int, default=200000, help="Maximum exact joint-association events per connected JPDA component before row-wise fallback")
