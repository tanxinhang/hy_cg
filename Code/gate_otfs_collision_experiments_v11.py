...
V11 updates:
  - Keeps V10 CRN-tracking fixes and multiprocessing support.
  - Adds output-safety controls (--fresh-out-dir) and run manifests to avoid
    mixing figures/CSVs from different runs.
  - Speeds up Gate-1/supplement evaluation by computing only the selected
    detector unless --save-all-detectors or detector-mode=both is requested.
  - Adds --save-diagnostics so verbose tracking diagnostic CSVs are optional.
  - Keeps compact, paper-facing tracking summaries while retaining full raw
    tracking_trials.csv as the reproducible source table.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD task-edge collision modeling (V11 faster detectors + clean outputs)")
