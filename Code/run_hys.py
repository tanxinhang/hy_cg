...
def run_one(h: str) -> None:
    ...
    cmd = [
        ...
        "--num-workers", "10",
        "--fresh-out-dir",
        "--out-dir", f"sweep_hys_{h}",
    ]

    print("=" * 80)
    print(f"Running hysteresis margin h={h}")
    print(" ".join(cmd))
    print("=" * 80)
...
def main() -> None:
    h_list = ["0", "0.02", "0.05", "0.10", "0.20"]

    for h in h_list:
        run_one(h)

    print("\nAll hysteresis sweeps finished successfully.")
