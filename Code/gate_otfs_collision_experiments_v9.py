...
V9 updates:
  - Keeps all V8 fixes and diagnostics.
  - Adds Monte-Carlo-level multiprocessing via --num-workers.
  - Parallelizes the prediction-robustness and closed-loop tracking gates,
    which are the most expensive gates in the current workflow.
  - Adds cache cleanup after each worker job to avoid unbounded actual-OTFS
    kernel/template cache growth in long multi-frame runs.
  - Uses stable method-specific RNG offsets instead of Python hash(), so
    multiprocessing results remain reproducible across processes.
...
def _tracking_worker(job: Tuple[SimConfig, int]) -> List[Dict[str, float]]:
    ...
    for frame in range(cfg.num_frames):
        ...
        for method in methods:
            st = states[method]
            rng = make_rng(cfg.seed + 73000 + 10000 * mc + 100 * frame + stable_method_offset(method))
            ...
            pd_vec = tgt_sorted["pd_target"].to_numpy(dtype=float)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD task-edge collision modeling (V9 with MC multiprocessing)")
