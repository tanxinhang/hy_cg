...
# -*- coding: utf-8 -*-
"""
Gate experiments for OTFS task-edge DD-angle collision modeling.

This script is intentionally focused on two go/no-go questions:

Gate 1: Does task-edge DD-angle collision translate into independent full-link
...
V8 updates:
  - Keeps all V7 fixes and diagnostics.
  - Adds a prediction-robustness gate: assignments are computed from noisy
    track predictions and evaluated on the true scene.
  - Adds a multi-frame tracking gate: track prediction -> assignment -> OTFS
    sensing -> simplified track update.
  - Adds optional switching cost for closed-loop assignment stability.
  - Main line is now OTFS DD task-edge collision; the angle gate is retained as
    an ablation/refinement diagnostic.
...
def evaluate_assignment_full_link(cfg: SimConfig, sc: Scenario, z: np.ndarray, rng: np.random.Generator) -> Dict[str, float]:
    ...
    return dict(
        pd_mean=float(np.mean(primary)),
        pd_p05=float(np.percentile(primary, 5)),
        pd_min=float(np.min(primary)),
        pd_energy_mean=float(np.mean(pd_energy_arr)),
        pd_energy_p05=float(np.percentile(pd_energy_arr, 5)),
        pd_matched_mean=float(np.mean(primary if mode_det != "energy" else pd_noncoh_arr)),
        pd_matched_p05=float(np.percentile(primary if mode_det != "energy" else pd_noncoh_arr, 5)),
        pd_coherent_matched_mean=float(np.mean(pd_coherent_arr)),
        pd_coherent_matched_p05=float(np.percentile(pd_coherent_arr, 5)),
        pd_noncoh_matched_mean=float(np.mean(pd_noncoh_arr)),
        pd_noncoh_matched_p05=float(np.percentile(pd_noncoh_arr, 5)),
...
def evaluate_assignment_targets(cfg: SimConfig, sc: Scenario, z: np.ndarray, rng: np.random.Generator) -> pd.DataFrame:
    ...
    It computes each assigned task's detection probability with the same detector
    primitives as evaluate_assignment_full_link(), then aggregates by target.
    The function is intentionally lighter than a full tracker but keeps the
    assignment->actual-link separation intact.
    """
    rows = []
    tasks = task_edge_list(z)
    for q in range(sc.Q):
        ...
        if not task_js:
            rows.append(dict(target=q, pd_target=0.0, rmse_pos_proxy_m=1e6, rmse_vel_proxy_mps=1e6,
                             full_cross_norm_mean=np.inf, num_task_receivers=0))
            continue
        pds = []
        pos_rmses = []
        vel_rmses = []
        ...
        for j in task_js:
            ...
            for i, qp in tasks:
                ...
                ov = dd_overlap_full(cfg, k_src, l_src, k_prot, l_prot)
                # Main DD-collision line: angle is optional/diagnostic and usually one.
            ...
            # Rough physical conversion: range error from monostatic delay proxy;
            # velocity error from Doppler error.  Floors keep the simplified tracker
            # from claiming unrealistically precise measurements.
            ...
            pds.append(pd_val)
            cross_norms.append(I / (S_eff + cfg.snr_floor + EPS))
        rows.append(dict(
            target=q,
            pd_target=float(np.mean(pds)),
            pd_target_p05=float(np.percentile(pds, 5)),
            rmse_pos_proxy_m=float(np.mean(pos_rmses)),
            rmse_vel_proxy_mps=float(np.mean(vel_rmses)),
            full_cross_norm_mean=float(np.mean(cross_norms)),
        ...
        for vel_sig in vel_list:
            for mc in range(cfg.pred_mc):
                ...
                for method in methods:
                    ...
                    row = dict(mc=mc, method=method, pred_pos_sigma_m=pos_sig, pred_vel_sigma_mps=vel_sig,
                               feasible_pred=is_feasible(cfg, z, v), coverage_ratio=float(np.mean(z.sum(axis=0) >= cfg.k_tgt_min)))
                    row.update(summ)
                    row.update(ev)
                    records.append(row)
    df = pd.DataFrame(records)
...
def run_tracking_experiment(cfg: SimConfig, out: Path) -> None:
    ...
    for mc in range(cfg.tracking_mc):
        ...
        states = {}
        for method in methods:
            rng = make_rng(cfg.seed + 71000 + 1000 * mc + abs(hash(method)) % 997)
            states[method] = dict(
                track_pos=true0_pos + rng.normal(0.0, cfg.track_init_pos_std_m, size=true0_pos.shape),
                track_vel=true0_vel + rng.normal(0.0, cfg.track_init_vel_std_mps, size=true0_vel.shape),
                miss=np.zeros(cfg.tracking_Q, dtype=int),
                z_prev=None,
            )
            states[method]["track_pos"][:, 2] = cfg.target_alt_m
            states[method]["track_vel"][:, 2] = 0.0
        for frame in range(cfg.num_frames):
            ...
            for method in methods:
                st = states[method]
                rng = make_rng(cfg.seed + 73000 + 10000 * mc + 100 * frame + abs(hash(method)) % 997)
                ...
                z = assign_by_method(cfg, method_assign, sc_pred, U, v, rng, z_prev=z_prev)
                tgt = evaluate_assignment_targets(cfg, sc_true, z, rng)
                pd_vec = tgt.sort_values("target")["pd_target"].to_numpy(dtype=float)
                pos_std_vec = tgt.sort_values("target")["rmse_pos_proxy_m"].to_numpy(dtype=float)
                vel_std_vec = tgt.sort_values("target")["rmse_vel_proxy_mps"].to_numpy(dtype=float)
                detected = rng.random(cfg.tracking_Q) < pd_vec
                for q in range(cfg.tracking_Q):
                    if detected[q]:
                        meas_pos = sc_true.tgt_pos[q] + rng.normal(0.0, pos_std_vec[q], size=3)
                        meas_vel = sc_true.tgt_vel[q] + rng.normal(0.0, vel_std_vec[q], size=3)
                        meas_pos[2] = cfg.target_alt_m
                        meas_vel[2] = 0.0
                        alpha_pos = 0.65
                        alpha_vel = 0.55
                        st["track_pos"][q] = alpha_pos * meas_pos + (1 - alpha_pos) * pred_pos[q]
                        st["track_vel"][q] = alpha_vel * meas_vel + (1 - alpha_vel) * pred_vel[q]
                        st["miss"][q] = 0
                    else:
                ...
                records.append(dict(
                    ...
                    full_cross_norm_mean=float(tgt["full_cross_norm_mean"].mean()),
    ...
    summary = df.groupby(["frame", "method"]).agg({
        "mean_track_pos_rmse_m": "mean", "p90_track_pos_error_m": "mean", "mean_track_vel_rmse_mps": "mean",
        "track_loss_rate": "mean", "mean_pd_target": "mean", "p05_pd_target": "mean",
        "full_cross_norm_mean": "mean", "switch_rate": "mean", "coverage_ratio": "mean",
    }).reset_index()
    summary.to_csv(out / "tracking_summary.csv", index=False)
    for metric, ylabel, fname in [
        ("mean_track_pos_rmse_m", "Track position RMSE (m)", "tracking_pos_rmse_time.png"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gate experiments for OTFS DD-angle task collision modeling")
