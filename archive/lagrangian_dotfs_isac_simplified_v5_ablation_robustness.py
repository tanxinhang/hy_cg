...
# -*- coding: utf-8 -*-
"""
Lagrangian-guided simplified DOTFS-ISAC cooperative sensing simulation (v5).
...
def evaluate_detection(
    ...
    Overall/system P_FA keeps all target hypotheses in the potential denominator.
    Inactive targets generate no fused statistic and therefore no false alarms.
    This quantity is useful as a system-level false-alarm rate, but it is not
    the detector calibration rate when some targets are inactive.
    """
    detected = 0
...
def print_summary(summary: Dict[str, Dict[str, Any]]) -> None:
    ...
    for method, s in summary.items():
        ...
        print(f"  P_FA overall           : {s['P_FA_overall']:.4f} "
...
def default_config() -> SimConfig:
    return SimConfig(
        ...
        use_comm_error_calibration=True,
        comm_error_model="erasure",
        use_target_priority=True,
        use_delay_price=True,
        seed=2026,
        verbose=True,
    )
