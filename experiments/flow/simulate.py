...
from isac_sim.core.config import Config, Link, apply_overrides, validate_config
...
from experiments.methods import C2F_METHODS, DEFAULT_METHODS, METHOD_RNG_OFFSETS, MethodName, fusion_weight_mode_for_method
...
from isac_sim.cooperation.primitives import feasible_links_for_target, sensing_only_links_for_target
...
def draw_h1_soft_stat(
    ...
    from isac_sim.sensing.soft_channel import draw_received_soft_stat
...
def evaluate_detection(
    ...
    *,
    ...
    """
    from isac_sim.sensing.soft_channel import draw_received_full_llr, draw_received_soft_vector
...
def total_overhead_bits(
    ...
    from isac_sim.sensing.model import packet_bits_for_target
...
def total_overhead_delay_s(
    ...
    * ``slot``:      conflict-graph colouring (shared transmitter or receiver
    ...
    """
    from isac_sim.sensing.fbl import report_latency_s
    from isac_sim.cooperation.reporting import slot_schedule
...
def build_slot_tables(
    ...
    conflict-graph colouring (:func:`isac_sim.cooperation.reporting.slot_schedule`).  Only
...
def _coordinated_c2f_selection(
    ...
    *,
    ...
    """
    from experiments.coordination import illuminator_mask, require_mask_covers_schedule
...
def run_method_on_trial(
    ...
    """
    from isac_sim.scenario.belief import truth_captured_links
    ...
    if method == "proposed_c2f_adaptive_pd_fusion_polish":
        from experiments.exploratory.fusion_polish import minimize_fixed_set_reports
    ...
    capture_rate = 1.0
    if belief_mode:
        from isac_sim.scenario.belief import belief_capture_rate
...
def _measure_trial_certificate(cfg, geom, geom_belief, base_truth, index):
    ...
    ctx = cx.ReceiverContext.from_trial(
        ...
        arm=cfg.cancellation.production_wire_arm,
    )
    ...
    return TrialCertificate(
        ...
        retention=np.asarray(
            meas.as_retention(
                per_target=cfg.scale.Q,
                source=cfg.cancellation.production_wire_retention,
            ),
            dtype=float,
        ).copy(),
    )
...
def _trial_certificate_scope(cfg, geom, geom_belief, base_truth, index):
    ...
    是"选哪些链路"，不是"接收机长什么样"。漏掉任何一个方法就等于让它在
    冻结常数的世界里被评估。
    """
...
def run_one_trial(
    ...
    if cfg.prior.belief_mode:
        ...
        # 证书作用域必须覆盖**选择侧**：选择用的 ``tables_belief`` 若在没有
        ...
        with _trial_certificate_scope(
            ...
            # Direction 2 gate: let the scheduler see the *lower-bound* gain on
            # the belief position ball instead of the (optimistic) point
            # estimate.  Detection still runs on ``base_truth``; only the
            # scheduling view is pessimised.  Off by default.
            if cfg.prior.robust_geometry_for_scheduler:
                from isac_sim.scenario.belief import geometry_robust_base

                base_belief = geometry_robust_base(cfg, base_belief)
