...
:func:`isac_sim.core.config.apply_overrides`), adding an ablation is a one-line
...
def robustness(cfg: Config, axis: str = "comm_model", values: List[Any] | None = None) -> List[Dict[str, Any]]:
    ...
    (``radio.residual_direct_factor``, legacy model only).

    .. note::
       曾有一条 ``direct_cancellation`` 轴扫 ``interference.direct_cancellation_db``。
       那个字段已被删除 —— 它没有接收机实现支撑却撑着 SINR 的整个分母 ——
       所以这条轴不再存在。对消深度现在是接收机的**实测输出**，不是扫描轴。
    """
    if values is None:
...
def waveform_check(
    ...
    """
    from isac_sim.sensing.waveform import sweep_compare_analytic_vs_psf
...
def waveform_detection(
    ...
    from isac_sim.sensing.waveform import waveform_llr_detection_check
...
def oracle_gap(
    ...
    from isac_sim.detection.oracle import greedy_objective, oracle_exhaustive
...
def runtime(
    ...
    from isac_sim.sensing.model import build_base_gains, compute_link_tables, generate_geometry
    from isac_sim.detection.oracle import oracle_exhaustive
    from experiments.selection import select_all_neighbor, select_c2f, select_lagrangian, select_topk_baseline
...
# --------------------------------------------------------------------------
# 曾有一个 ``INTERFERENCE_CANCEL_DB = [0..80]`` 扫描表在这里，用来扫
# ``interference.direct_cancellation_db``。那个字段已被删除：它没有接收机实现
# 支撑，却撑着感知 SINR 的整个分母（取 0 dB 时 P_D 掉到 P_FA），所以挂在它
# 上面的任何曲线都是记账而不是性能。对消深度现在是 TP-UIC 的**实测输出**。
...
def _interference_diagnostics(cfg: Config, n_trials: int = 20) -> Dict[str, float]:
    ...
    * ``sense_inr_db``    -- interference-to-noise ratio at the sensing
      receiver, i.e. ``kappa_dc * field / n0``.  Since the cancellation
      constant was deleted, the un-wired default is ``kappa_dc = 1``: this
      column is therefore the **uncancelled** INR, i.e. what the receiver is
      actually asked to suppress.  A cancelled value exists only once a
      measured residual is injected, and then it shows up in
      ``sense_inr_table_db`` (read back from the link table itself).
    ...
    * ``sense_sinr_no_cancel_db`` -- SINR it would get with *no* direct-path
    ...
    """
    from isac_sim.sensing.model import (build_base_gains, compute_link_tables, denominator_guard,
    ...
    # 删除 direct_cancellation_db 之后，未注入实测残余的链路就是**没有对消**。
    # 想看"对消之后"必须注入实测残余；那时 rinr 表本身会变，读 sense_inr_table_db。
    kappa = 1.0
...
def interference_consistency(
    ...
    Compares four bookkeeping variants at one operating point: the legacy
    ...
    .. note::
       曾经还有 Part 2：扫 ``kappa_dc`` 去找"感知接收机必须做到多少对消"。
       那个扫描建立在被删除的 ``interference.direct_cancellation_db`` 上 ——
       一个没有接收机实现支撑、却撑着整个分母的常数。近远比（``near_far_db``）
       仍然是同一个几何量，但它现在是**要由 TP-UIC 实测去兑现的需求**，不是
       可以扫的旋钮；对消后的 INR 请读 ``sense_inr_table_db``。
    """
    rows: List[Dict[str, Any]] = []
...
def submodularity(
    ...
    from isac_sim.sensing.model import build_base_gains, compute_link_tables, generate_geometry
    from isac_sim.cooperation.reporting import assign_fusion_nodes
    from audits.theory import curvature_reference_bound, submodularity_audit
...
def same_objective_gap(
    ...
    from isac_sim.sensing.model import build_base_gains, compute_link_tables, generate_geometry
    from experiments.selection import select_lagrangian
    from audits.theory import same_objective_oracle, task_objective
    from isac_sim.cooperation.reporting import assign_fusion_nodes
