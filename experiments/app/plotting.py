...
from experiments.app.naming import method_order, paper_label
...
def plot_interference_consistency(rows, out_dir: Path) -> None:
    ...
    # 曾有一个 (b) 面板画 P_D / SINR 对 ``kappa_dc`` 的曲线。那条曲线建立在被删除的
    # ``interference.direct_cancellation_db`` 上，所以扫描行不再产生，面板随之退役。
    fig, ax = plt.subplots(1, 1, figsize=(6.2, 3.8))

    # bookkeeping variants: the two INR levels side by side
    if book:
