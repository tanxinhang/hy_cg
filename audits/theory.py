...
from isac_sim.cooperation.primitives import feasible_links_for_target, link_cost_ms
...
def worst_case_objective(
    ...
    """``Phi = min_q min(P_D,q, P_D,req)`` —— 提案 (P1) 里被接受的那个 ``t``。

    与 :func:`task_objective` 的分工是明确的，别混：

    * ``task_objective`` 是**贪婪排序用的势**。纯 min 的边际恒为 0 会让贪婪
      一步都走不动（实测选中链路 12.4 -> 0），所以势用 leximin
      （``fusion.leximin``，升序加权和，Schur-凹、边际恒正）。
    * 本函数是**接受判据**。提案 §19 要求被单调化的量就是最差目标检测概率
      本身，不含任何价格项、不含平滑 —— 所以这里就是精确的 min。

    两者在 ``selector.maxmin_objective`` 打开后应当**同向**：势负责"下一步
    选哪条"，Phi 负责"这一步要不要"。
    """
    pds = np.array(
        [
...
def same_objective_oracle(
    ...
    :func:`isac_sim.detection.oracle.oracle_exhaustive` (which maximised ``sum_q D_q``).
    """
    Q = cfg.scale.Q
