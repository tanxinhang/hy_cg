...
adding a new sensing mechanism. The main results remain the archived V1
experiments. New component audits are labeled separately and do not replace the
...
RCS of $50~\mathrm{m}^2$; detection uses the unknown current state. Blind first
...
main run, not relabeling an audit.
...
crossing zero is not proof of equivalence or non-inferiority. The archived
V1 active-target false-alarm estimate is 0.04975, near the specified 0.05.
...
\begin{itemize}
\item \texttt{results\_target\_local\_v1/main/main.csv}: archived 1000-trial main comparison.
...
\caption{Archived exploratory envelope, 100 trials per condition.
...
No additional uncertainty bands are inferred from unavailable trial-level data.}
\end{figure}
\clearpage
...
requires a separate benchmark. Released C2F and the MC=1000 headline
remain unchanged.
