...
greedy replay is aligned with the implemented detector. This decomposition
removes circular dependence between fusion planning and link selection. The
distance rule is a prediction-only heuristic, not a joint optimum. The inner
detector-predicted utility directly prices remote reports and independently
determines how many observations to select.

Our main contributions are as follows.

\begin{itemize}
	\item We formulate decomposed target-specific fusion planning and triplet-level
	...
	\item We combine this outer planning layer with local-evidence retention,
	communication-error calibration,
	an adaptive coarse frontier, waveform-derived DD refinement, and detector-aligned
	greedy replay. The sensing and reporting graphs are distinct, and the
	detector-PD selector no longer inherits a budget from a reference algorithm.
